...
void push(char **Stack, int *top, int *size, char key)
{
    if (isFull(*top, *size))
    {
        //printf("\nThe stack is full, reallocating size;");
        *Stack = (char *)realloc(*Stack, sizeof(char) * (*size) * 2);
        *size *= 2;
    }
    (*top)++;
    (*Stack)[*top] = key;
...
void convert(char **infix, char **postfix, char **Stack, int *top, int *size)
{
    char *ptr = *postfix;
    char *ptr2 = *infix;
...
int main()
{
    ...
    convert(sendInfix, sendPostfix, &Stack, &top, &size);
